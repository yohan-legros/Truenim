...
public class GameState {
     ...
     *
     * @see Board#auTourDe(int, HumanPlayer, HumanPlayer)
     ...
     */
    public static void inGame (GameState plateau, HumanPlayer player1, HumanPlayer player2, int n) {
        int tours = 0;
        for (int i = 0; i < plateau.getBoard().length; i++) {
            ...
            while (plateau.getBoard()[i] != 0) {
                String playing_player = Board.auTourDe (tours, player1, player2);
                Console.showBoard (S);
        ...
        Console.showWinner (Board.winner (tours, player1.getName(), player2.getName()));
    }
